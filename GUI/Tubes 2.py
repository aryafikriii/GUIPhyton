...
def AddForm():
    ...
    tanggal=str(cal.selection_get())
    tk.Button(win, text='Tambah', command=lambda: addTodo(win, tanggal, jam, menit, judul, keterangan), fg='white', bg='blue').grid(row=6, column=0)
...
cal = Calendar(root, font='Impact 13', selectmode='day', locale='id_ID', cursor='hand1', background="black", disabledbackground="white", bordercolor="black", 
...
treev.grid(row=8, column=0, sticky='NS', rowspan=4, columnspan=1)
scrollBar = tk.Scrollbar(root, orient='vertical', command=treev.yview)
scrollBar.grid(row=8, column=2, sticky='ENS', rowspan=4)
...
btnAdd = tk.Button(root, text='Tambah', width=10, height=3, command=AddForm, fg='white', bg='blue')
btnAdd.grid(row=8, column=1, sticky='N')

btnDel = tk.Button(root, text='Hapus', width = 10, height=3, command=delTodo, fg='white', bg='red')
btnDel.grid(row=9, column= 1, sticky= 'N')

btnLoad = tk.Button(root, text='Load', width = 10, height=3, command=LoadTodos, fg='black', bg='yellow')
btnLoad.grid(row=10, column= 1, sticky= 'S')

btnSave = tk.Button(root, text='Save', width = 10, height=3, command=SaveTodos, fg='white', bg='green')
btnSave.grid(row=11, column= 1, sticky= 'S')
